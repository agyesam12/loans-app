...
def member_required(view_func, denied_url="home"):
    @functools.wraps(view_func)
    def wrapper(request, *args, **kwargs):
        if (request.user.is_authenticated and request.user.is_member) or (request.user.is_authenticated and request.user.is_admin):
            return view_func(request, *args, **kwargs)
        messages.info(request, "Page not found")
        return redirect(denied_url)  
    return wrapper
...
def prophet_required(view_func, denied_url="home"):
    @functools.wraps(view_func)
    def wrapper(request, *args, **kwargs):
        if (request.user.is_authenticated and request.user.is_prophet) or (request.user.is_authenticated and request.user.is_admin):
            return view_func(request, *args, **kwargs)
        messages.info(request, "Page not found")
        return redirect(denied_url)  
    return wrapper
